...
class Linked_List:
    ...
    def insert_element_at(self, val, index):
        """Inserts an element or value at a specified index. First Value is indexed as the 0th position."""
    ...
    def remove_element_at(self, index):
        """Removes an element at a specified index. First value is indexed as the 0th position."""
        # If the index is invalid, raise an IndexError exception.
        if index < 0 or index >= self._size:
            raise IndexError
        if self._size == 0:
            raise ValueError('Cannot remove value, list is already empty')
        to_return = self.get_element_at(index)   ###COME BACK TO,  should be + 1####
        current = self._header
    ...
    def get_element_at(self, index):
        """Gets an element or value at a specified index, first value in list = 0th position"""
...
""" # Unit tests make this unnecessary
    ...
    try:
        ...
        ll.remove_element_at(0)     # should fail, the list is empty
    ...
    print("------------------------------------")
    try:
        ll.append_element(0)                    # should append the value zero
        a = ll.remove_element_at(0)             # should remove value at 0-th position, just happens to be zero
        print("Element removed = " + str(a))    # should return: 0
        ll.append_element(1)                    # should append
        ll.insert_element_at(0, 0)              # should add zero at the 0-th position (first position)
    ...
    try:
        ...
        ll.insert_element_at(8, 4)    # should fail, out of index
    ...
    try:
        ...
        ll.remove_element_at(0)   # should work, removes number 0 at zero-th index
        ll.remove_element_at(5)   # should fail, out of index
        ll.remove_element_at(-1)  # should fail, out of index
    except IndexError:
        print("Error(s) Caught: Please enter a valid index.")
    print("Linked List after removing elements: " + str(ll))  # should return: [ 1, 2 ]
    print("Size of the Linked List: " + str(len(ll)))         # should return: 2
    print(" ")
    print(" ")
    ...
    print("--------------------------------")
    try:
        print("Getting Value at Zero-th Position: " + str(ll.get_element_at(0)))
        print("Getting Value in the First Position: " + str(ll.get_element_at(1)))
        print("Getting Value in the (Invalid) Second Position: " + str(ll.get_element_at(2)))  # should fail
