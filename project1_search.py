###############################################
# Skyler Kuhn
# BNFO 501: Project 1 Program
# Implementing a Sequential and Binary search
################################################
...
seqSearchOutputFH = open("project1_sequentialSearch_output.txt", "w")
...
# Now let's see if the query is in the data list!
binSearchOutputFH = open("project1_binarySearch_output.txt", "w")
binSearchOutputFH.write("SearchTime\t\t\t\tStartTime\t\t\t\tQuery\n")  # writing a pretty header
print("Binary Search Timing")
datalist = sorted(datalist)  # this needs to be sorted before we can start binary search
for query in querylist:
    start = time.clock()  # starts the timer
